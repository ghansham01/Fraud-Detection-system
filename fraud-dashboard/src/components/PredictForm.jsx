...
function PredictForm({ setResult, setLoading, setError, loading }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || "API se connect nahi ho pa raha — check karo API chal rahi hai?"
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
      ...
      <div>
        ...
        >
          {mode === "simple" ? "Show V1–V28 fields" : "Hide V1–V28 fields"}
        </button>
      </div>
